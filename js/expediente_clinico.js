/**
 * ====================================================================================
 * BLOQUE 10: EXPEDIENTE CLÍNICO SOBERANO V29.0 "ACCESIBILIDAD TOTAL"
 * Triaje Biomecánico, Cuadrículas A11y (ARIA), Panel de Firma Dedicado, PDF 4 Hojas.
...
window.ExpedienteEngine = {
    ...
    clinicalDict: {
        precauciones: { title: "A. Condiciones y Precauciones Físicas", icon: "fa-shield-heart", items: [
            ...
            { id: "os", label: "Osteoporosis / Osteopenia", desc: "Pérdida de masa o descalcificación ósea. Hay riesgo por presión fuerte." },
        ...
        alergias: { title: "C. Alergias y Sensibilidad", icon: "fa-hand-dots", items: [
            ...
            { id: "ca", label: "Intolerancia al Calor", desc: "Hipersensibilidad a la termoterapia (Ej. piedras calientes)." },
    ...
    accessibilityDict: [
        ...
        { id: "co", label: "Dificultad para entender instrucciones", icon: "fa-brain" },
        ...
        { id: "dp", label: "Dolor severo al cambiar posición", icon: "fa-arrows-rotate" },
        { id: "pa", label: "Necesidad de pausas constantes", icon: "fa-pause" },
        { id: "am", label: "Requiere ingresar con acompañante", icon: "fa-user-group" }
    ],

    ajustesDict: [
        { id: "ex", label: "Brindar explicaciones claras y lentas" },
        { id: "ls", label: "Utilizar lenguaje simple y directo" },
        { id: "ap", label: "Brindar apoyo para leer/llenar formatos" },
        { id: "tg", label: "Proporcionar textos más grandes" },
        { id: "il", label: "Mantener intensidad de luz baja" },
        { id: "mr", label: "Mantener ruido ambiente al mínimo" },
        { id: "mp", label: "Garantizar mayor privacidad visual" },
        { id: "as", label: "Brindar apoyo físico en camilla" },
        { id: "cp", label: "Realizar cambios de postura graduales" },
        { id: "ip", label: "Aplicar menor presión terapéutica" },
        { id: "em", label: "Evitar zonas o maniobras dolorosas" },
        { id: "pp", label: "Programar pausas durante la sesión" }
    ],
    ...
    injectStyles: function() {
        ...
        style.innerHTML = `
            ...
            .btn-success { background: linear-gradient(135deg, #00b09b, #00796b); color: white; box-shadow: 0 10px 30px rgba(0,176,155,0.4); width:100%; }
            .btn-success:disabled { background: #555; color:#888; box-shadow:none; cursor:not-allowed; transform:none; }
            ...
            .a11y-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(240px, 1fr)); gap: 15px; }
            .a11y-grid-btn { 
                background: rgba(255,255,255,0.03); border: 2px solid rgba(255,255,255,0.1); border-radius: 14px; 
                padding: 20px; cursor: pointer; transition: all 0.3s ease; display: flex; flex-direction: column; 
                align-items: flex-start; text-align: left; color: white; width:100%; outline:none;
            }
            .a11y-grid-btn:focus-visible { border-color: var(--valtara-oro); box-shadow: 0 0 0 4px rgba(242,201,76,0.3); }
            .a11y-grid-btn:hover { background: rgba(255,255,255,0.08); border-color: rgba(255,255,255,0.3); }
            
            .a11y-grid-btn.active { 
                background: rgba(0, 255, 255, 0.08); border-color: var(--valtara-cian-brillante); box-shadow: 0 5px 20px rgba(0,255,255,0.15); 
            }
            .a11y-grid-btn.danger.active { 
                background: rgba(255, 85, 85, 0.08); border-color: #ff5555; box-shadow: 0 5px 20px rgba(255,85,85,0.15); 
            }
            .a11y-grid-btn.warning.active { 
                background: rgba(242, 201, 76, 0.08); border-color: var(--valtara-oro); box-shadow: 0 5px 20px rgba(242,201,76,0.15); 
            }

            .a11y-btn-header { display: flex; align-items: center; gap: 15px; margin-bottom:10px; width:100%; }
            .a11y-btn-title { font-weight: 900; font-size: 1.05rem; flex:1; line-height:1.3; }
            .a11y-btn-desc { font-size: 0.85rem; color: #aaa; margin-bottom: 15px; font-weight:300; line-height:1.4; }
            .a11y-status-badge { font-family:monospace; font-weight:bold; font-size:0.8rem; padding:6px 12px; border-radius:8px; background:rgba(255,255,255,0.05); color:#888; width:100%; text-align:center; transition:0.3s; }
            
            /* MODAL DE FIRMA DEDICADO */
            .sig-modal-overlay { position: fixed; top:0; left:0; width:100vw; height:100vh; background:rgba(0,0,0,0.95); z-index:9999999; display:none; flex-direction:column; justify-content:center; align-items:center; padding:20px; }
            .sig-modal-overlay.active { display:flex; }
            .sig-modal-content { background:#fff; width:100%; max-width:600px; border-radius:20px; padding:20px; text-align:center; }
            .sig-canvas { width: 100%; height: 300px; background: #fdfdfd; border-radius: 10px; border: 2px dashed #ccc; cursor: crosshair; touch-action: none; margin-bottom:20px; }
    ...
    injectModal: function() {
        ...
        <div id="expediente-overlay" class="exp-overlay">
            <div class="exp-container">
                ...
                <div class="exp-footer">
                    ...
                    <div id="exp-loader" style="display:none; color:var(--valtara-cian-brillante); font-family:monospace; font-size:0.9rem; flex:2; text-align:center; letter-spacing:2px; font-weight:bold;"><i class="fa-solid fa-lock fa-fade"></i> CIFRANDO DOCUMENTO...</div>
        ...
        <!-- MODAL DEDICADO PARA FIRMA AUTÓGRAFA -->
        <div id="signature-modal" class="sig-modal-overlay">
            <div class="sig-modal-content">
                <h3 style="color:#050508; font-size:1.5rem; margin-top:0; margin-bottom:10px; font-weight:900; text-transform:uppercase;">Firma Digital Autógrafa</h3>
                <p style="color:#666; font-size:0.9rem; margin-bottom:20px;">Por favor, dibuje su firma dentro del recuadro usando su dedo.</p>
                
                <canvas id="sig-canvas" class="sig-canvas" aria-label="Lienzo para dibujar firma autógrafa"></canvas>
                
                <div style="display:flex; justify-content:space-between; gap:10px;">
                    <button onclick="ExpedienteEngine.closeSignaturePad(false)" style="padding:15px; background:#f5f5f5; color:#555; border:1px solid #ccc; border-radius:10px; font-weight:bold; flex:1; cursor:pointer;">Cancelar</button>
                    <button onclick="ExpedienteEngine.clearCanvas()" style="padding:15px; background:#fff0f0; color:#c62828; border:1px solid #ffcdd2; border-radius:10px; font-weight:bold; flex:1; cursor:pointer;"><i class="fa-solid fa-eraser"></i> Borrar</button>
                    <button onclick="ExpedienteEngine.closeSignaturePad(true)" style="padding:15px; background:#000; color:#D4AF37; border:none; border-radius:10px; font-weight:900; flex:2; cursor:pointer;">GUARDAR FIRMA</button>
                </div>
            </div>
        </div>
        `;
    ...
    renderWizard: function() {
        ...
        let zonasHTML = this.zonasList.map(z => this.buildGridToggleCard(z, 'zonas', null, z, 'fa-child-reaching', 'warning')).join('');
        ...
        // PASO 5: Clínica (Iterar Diccionario en Cuadrícula A11y)
        let clinicas = '';
        for (const [catKey, category] of Object.entries(this.clinicalDict)) {
            clinicas += `<h3 style="color:var(--valtara-cian-brillante); margin: 30px 0 15px 0; border-bottom:1px solid rgba(0,255,255,0.2); padding-bottom:10px; font-size:1.3rem; text-transform:uppercase; letter-spacing:1px;"><i class="fa-solid ${category.icon}"></i> ${category.title}</h3><div class="a11y-grid">`;
            category.items.forEach(item => {
                const isDanger = catKey === 'precauciones' || catKey === 'infecciosas';
                clinicas += this.buildGridToggleCard(item.label, 'clinical', catKey, item.id, isDanger ? 'fa-triangle-exclamation' : 'fa-notes-medical', isDanger ? 'danger' : 'warning', item.desc);
            });
            clinicas += `</div>`;
        }
        html += this.buildStep(5, `
            ...
            <p style="color:#aaa; font-size:1rem; margin-bottom:20px; line-height:1.5;">La masoterapia interactúa con los sistemas circulatorio y nervioso. Seleccione <strong>únicamente</strong> las condiciones con las que ha sido diagnosticado recientemente o de forma crónica.</p>
            ${clinicas}
        `);
        ...
        // PASO 7: Accesibilidad (Cuadrícula A11y)
        let accHTML = this.accessibilityDict.map(i => this.buildGridToggleCard(i.label, 'accessibility', 'profile', i.label, i.icon, 'active')).join('');
        let ajsHTML = this.ajustesDict.map(i => this.buildGridToggleCard(i.label, 'accessibility', 'supports', i.label, 'fa-check', 'active')).join('');
        html += this.buildStep(7, `
            ...
            <h3 style="color:white; font-size:1.3rem; margin-bottom:15px; border-bottom:1px solid rgba(255,255,255,0.1); padding-bottom:10px;">Situaciones o Necesidades Especiales:</h3>
            <div class="a11y-grid" style="margin-bottom:30px;">${accHTML}</div>
            <h3 style="color:white; font-size:1.3rem; margin-bottom:15px; border-bottom:1px solid rgba(255,255,255,0.1); padding-bottom:10px;">Ajustes Razonables Sugeridos:</h3>
        ...
        // PASO 10: Legal y FIRMA (CON MODAL DEDICADO)
        html += this.buildStep(10, `
            ...
            <button onclick="ExpedienteEngine.toggleLegalTruth(this)" class="a11y-grid-btn ${this.formData.legal.truthOath ? 'active' : ''}" role="switch" aria-pressed="${this.formData.legal.truthOath}" style="margin-bottom:30px;">
                <div class="a11y-btn-header">
                    ...
                    <span class="a11y-btn-title">Acuerdo de Veracidad y Privacidad</span>
                </div>
                <span class="a11y-btn-desc">He leído y acepto los términos. Autorizo mi terapia.</span>
                <div class="a11y-status-badge" style="${this.formData.legal.truthOath ? 'background:rgba(0,255,255,0.1); color:var(--valtara-cian-brillante);' : ''}">${this.formData.legal.truthOath ? '✅ ACEPTADO' : '◻️ Sin aceptar'}</div>
            </button>

            <div style="background: rgba(242,201,76,0.05); border:1px solid rgba(242,201,76,0.3); padding:25px; border-radius:15px; text-align:center;">
                <h3 style="color:var(--valtara-oro); margin-top:0; font-size:1.3rem; text-transform:uppercase;">Identidad Biométrica</h3>
                <p style="color:#aaa; font-size:0.95rem; margin-bottom:20px;">Requerimos su firma autógrafa digital para certificar el documento.</p>
                
                <div id="firma-preview-container" style="display:${this.formData.legal.signatureData ? 'block' : 'none'}; margin-bottom:20px;">
                    <img id="firma-preview-img" src="${this.formData.legal.signatureData || ''}" style="max-width:100%; height:120px; background:white; border-radius:10px; padding:10px; border:2px solid var(--valtara-oro);">
                </div>

                <button onclick="ExpedienteEngine.openSignaturePad()" style="background:var(--valtara-oro); color:black; border:none; padding:15px 30px; border-radius:12px; font-weight:900; font-size:1.1rem; cursor:pointer; width:100%; max-width:400px; margin:0 auto; display:flex; justify-content:center; align-items:center; gap:10px;">
                    <i class="fa-solid fa-pen-nib"></i> ${this.formData.legal.signatureData ? 'Volver a Firmar' : 'ABRIR PANEL DE FIRMA'}
                </button>
            </div>
        `);
    ...
    // ====================================================================
    // MOTOR DE CUADRÍCULAS ACCESIBLES (A11y Grid Buttons)
    // ====================================================================
    buildGridToggleCard: function(label, cat, arrKey, val, icon, colorTheme, desc = '') {
        let isChecked = false;
        if(Array.isArray(this.formData[cat][arrKey])) {
            ...
            let str = this.formData[cat][arrKey] || "";
        ...
        const iconColor = isChecked ? colorVar : '#666';
        ...
        return `
        <button type="button" class="a11y-grid-btn ${activeClass}" role="switch" aria-pressed="${isChecked}" onclick="ExpedienteEngine.toggleGridItem(this, '${cat}', '${arrKey}', '${val}', '${label}', '${colorTheme}')">
            <div class="a11y-btn-header">
                <i class="fa-solid ${icon} icon-elem" style="font-size:2rem; color:${iconColor}; transition:0.3s;"></i>
                <span class="a11y-btn-title">${label}</span>
            </div>
            ${desc ? `<span class="a11y-btn-desc">${desc}</span>` : ''}
            <div class="a11y-status-badge" style="background:${badgeBg}; color:${badgeColor}; margin-top:auto;">${badgeText}</div>
        </button>`;
    },

    toggleGridItem: function(btn, cat, arrKey, val, label, colorTheme) {
        // 1. Lógica de Datos
        let isChecked = false;
        if(Array.isArray(this.formData[cat][arrKey])) {
            let arr = this.formData[cat][arrKey];
            const idx = arr.indexOf(val);
            if(idx > -1) { arr.splice(idx, 1); isChecked = false; }
            else { arr.push(val); isChecked = true; }
        } else if(arrKey === 'supports') {
            let str = this.formData[cat][arrKey] || "";
            let arr = str.split(', ').filter(i => i.trim() !== "");
            const idx = arr.indexOf(val);
            if(idx > -1) { arr.splice(idx, 1); isChecked = false; }
            else { arr.push(val); isChecked = true; }
            this.formData[cat][arrKey] = arr.join(', ');
        } else {
            this.formData[cat][arrKey][val] = !this.formData[cat][arrKey][val];
            isChecked = this.formData[cat][arrKey][val];
        }
        this.saveData();

        // 2. Manipulación Visual Directa (Cero recargas de página)
        const colorVar = colorTheme === 'danger' ? '#ff5555' : (colorTheme === 'warning' ? 'var(--valtara-oro)' : 'var(--valtara-cian-brillante)');
        const badgeBg = isChecked ? `rgba(${colorTheme==='danger'?'255,85,85':(colorTheme==='warning'?'242,201,76':'0,255,255')}, 0.1)` : 'rgba(255,255,255,0.05)';
        ...
        if(isChecked) {
            ...
            iconElem.style.color = '#666';
            badgeElem.style.background = 'rgba(255,255,255,0.05)';
            badgeElem.style.color = '#888';
            badgeElem.innerText = '◻️ Sin seleccionar';
        }

        // 3. Anunciador Auditivo Inclusivo
        this.announceA11y(`${label} ${isChecked ? 'ha sido marcado como verdadero' : 'ha sido desmarcado'}`);
    },
    ...
    // ====================================================================
    // PANEL DE FIRMA AISLADO Y TÁCTIL
    // ====================================================================
    openSignaturePad: function() {
        ...
        // Lógica de dibujo con preventDefault para móviles
        const getPos = (e) => {
            const rect = canvas.getBoundingClientRect();
        ...
        // Limpiar event listeners previos para no duplicar
        const clone = canvas.cloneNode(true);
        canvas.parentNode.replaceChild(clone, canvas);
        this.signaturePad = document.getElementById('sig-canvas');
        const newCtx = this.signaturePad.getContext('2d');
        newCtx.lineWidth = 3; newCtx.lineCap = 'round'; newCtx.strokeStyle = '#050508';
    ...
    bindEvents: function() {
        ...
        btnNext.addEventListener('click', () => {
            ...
            if(this.currentStep < this.totalSteps - 1) { 
                this.currentStep++; 
                this.renderWizard(); 
                this.announceA11y(`Paso ${this.currentStep + 1} cargado.`);
            }
        });
    ...
    updateUI: function() {
        // La barra inferior de navegación
        ...
        // Lógica de visibilidad del botón Atrás (Garantizado con Flexbox)
        const btnPrev = document.getElementById('exp-btn-prev');
        if(this.currentStep > 0) {
    ...
    // ====================================================================
    // MOTOR PDF MULTI-HOJA CON MARCA DE AGUA NIVEL BILLETE (V29.0)
    // ====================================================================
    generatePDF: function() {
        ...
        this.announceA11y("Iniciando cifrado y generación del documento PDF.");
